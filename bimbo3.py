...
def fast_q(x):
    ...
    i+=1
    print(i)
    return(np.nanpercentile(np.array(x), 50))

meds=data.groupby([c for c in cols if c!='Semana'])['demand_ratio'].agg(fast_q)
meds=meds.reset_index()
meds.rename(columns={'demand_ratio': 'median'}, inplace=True)

data=pd.merge(data, meds, left_on=[c for c in cols if c!='Semana'], right_on=[c for c in cols if c!='Semana'])
data.reset_index(drop=True, inplace=True)

cnts=data.groupby([c for c in cols if c!='Semana'])['demand_ratio'].size()
cnts=cnts.reset_index()
cnts.rename(columns={0: 'cnt'}, inplace=True)

data=pd.merge(data, cnts, left_on=[c for c in cols if c!='Semana'], right_on=[c for c in cols if c!='Semana'])

singles=data[data.cnt==1]

data=data[(data.cnt!=1) & (data.demand_m1.notnull()) &(data['median'].notnull())]

data['dr1'].ix[data['dr1'].isnull()]=data['median'].ix[data['dr1'].isnull()]
data['dr2'].ix[data['dr2'].isnull()]=data['median'].ix[data['dr2'].isnull()]

"""
# unique days
days=data.Semana.unique()
days_s=pd.Series(index=days)

data.index=data.Semana

def tpose(x):
    global i
    i+=1
    print(i)
    return([x[x.index==d] if len(x[x.index==d])==0 else None for d in days])
...
def tpose2(x):
    ...
    i+=1
    print(i)
    return(tuple(pd.concat((x,days_s), axis=1).ix[:,0]))

tp=data.groupby([c for c in cols if c!='Semana']).agg(tpose)
"""
...
features=[c for c in cols if c not in ['Semana']]+['dr1', 'dr2']
target='demand_ratio'
...
def scorer(estimator, X,y):
    ...
    resid=y-predicted
    return(sqrt(np.dot(resid, resid)/resid.shape[0]))
...
for i, d in enumerate(days):
    df_list.append(test[(test.Semana==d)])
    if i==0:
        df_list[i]['prediction']=clf_large.predict(df_list[i][features])
        df_list[i]['pred_norm']=np.exp(df_list[i]['Demanda_uni_equil']+df_list[i]['prediction'])
        df_list[i].sort_index(axis=1, inplace=True)
    else:
        df_list[i]=pd.merge(df_list[i], df_list[i-1][join_cols+['prediction', 'pred_norm']], \
        how='inner', left_on=list(join_cols), right_on=list(join_cols), suffixes=('','_d'))
        df_list[i]['dr2']=df_list[i]['dr1']
        df_list[i]['dr1']=df_list[i]['prediction']
        del df_list[i]['prediction']
        df_list[i]['prediction']=clf_large.predict(df_list[i][features])
        df_list[i]['pred_norm']=df_list[i]['pred_norm']*np.exp(df_list[i]['prediction'])
        df_list[i].sort_index(axis=1, inplace=True)
        
test=pd.concat(tuple(df_list), axis=0)
...
test=test[(test.Semana_d==test.Semana-1) | (test.Semana_d==test.Semana-2)]

test['prediction']=clf_large.predict(test[features])
